  ...
  async validate(idx?: number, block?: Block) {
    ...
    const inputValues = await Promise.all(
      this.inputs.map(async (input, i) => {
        ...
        let outputString = input.outpoint.toString();
        if (allOutputStrings.has(outputString)) {
          throw new Error(`Tx inputs share the same output, ${outputString}`);
        }
        allOutputStrings.add(outputString);
